...
if (computing == T){  
  ...
  for (p in samples){
    ...
    NMBA_p = if(!is.na(data_sub['NMBA'])){NMBA_p = NMBA_val}else{
      next
    }
    ...
    ## FO2_deno
    Obs_FO2deno = FO2_p[[1]] -  predict( FO2deno.cond, (as.matrix(cbind(PEEP_p,NMBA_p,Sev_p)) ))
    ...
    Obs_VTdeno = VT_p[[1]] - predict( VTdeno.cond, (as.matrix(cbind(RR_p,Age_p,NMBA_p, PEEP_p)) ))
    prob_VTdeno = dnorm(Obs_VTdeno, mean=0, sd=2)
    Obs_PEEPdeno = PEEP_p[[1]] - predict(PEEPdeno.cond, (as.matrix(cbind(RR_p,Age_p,NMBA_p)) ))
    ...
    fd_p = exp( log(prob_FO2deno) + log(prob_VTdeno) + log(prob_PEEPdeno) )
    sum_numer = 0 
    for (i in 1:Ni){
      mean_PP = predict(PP.cond, as.matrix(cbind(RR_p, Age_p, Sev_p, NMBA_p, KG_p)))
      PP_i = rnorm(n=1,mean=mean_PP, sd=6)
      prob_y = predict(Y.cond,as.matrix(cbind(RR_p, Sev_p, NMBA_p, KG_p, PP_i, PEEP_p, FO2_p, VT_p)))
      Obs_FO2 = FO2_p[[1]] - predict(FO2.cond, as.matrix(Age_p, Sev_p, NMBA_p, KG_p, PP_i, PEEP_p))
      ...
      Obs_PEEP = PEEP_p[[1]] - predict(PEEP.cond, as.matrix(RR_p, Age_p, Sev_P, NMBA_p, KG_p, PP_i))
      ...
      Obs_VT = VT_p[[1]] - predict(VT.cond, as.matrix(RR_p,Sev_p,NMBA_p,KG_p,PP_i,PEEP_p,FO2_p))
      prob_VT = dnorm(Obs_VT, mean=0, sd=2)
      sum_numer = sum_numer + prob_y * prob_FO2 * prob_PEEP * prob_VT
    }
    fn_p = sum_numer / Ni
    prob = fn_p / fd_p 
